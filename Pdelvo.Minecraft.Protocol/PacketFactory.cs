...
namespace Pdelvo.Minecraft.Protocol
{
    ...
    public static class PacketFactory
    {
        ...
        {
            return new PacketCreator(() => (Packet)Activator.CreateInstance(type));
        }
        ...
        private static PacketSender GetMonoSender(Type type)
        {
            ...
            int requiredVersion = PacketEndPoint.GetRequiredVersion(type, out throwOnRequired);
            int lastSupportedVersion = PacketEndPoint.GetLastSupportedVersion(type, out throwOnLast);
            return new PacketSender((stream, version, packet) =>SendPacket(packet, stream, version, requiredVersion, lastSupportedVersion, throwOnRequired, throwOnLast));
        }
        ...
        private static PacketSenderAsync GetMonoSenderAsync(Type type)
        {
            ...
            int requiredVersion = PacketEndPoint.GetRequiredVersion(type, out throwOnRequired);
            int lastSupportedVersion = PacketEndPoint.GetLastSupportedVersion(type, out throwOnLast);
            return new PacketSenderAsync((stream, version, packet) => SendPacketAsync(packet, stream, version, requiredVersion, lastSupportedVersion, throwOnRequired, throwOnLast));
        }
